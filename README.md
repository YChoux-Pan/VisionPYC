...
2026/8/30 功能完善（采集/预处理/流程验证）

- 文件菜单：新建 / 打开 / 保存流程（*.vpjson JSON 格式，算子参数持久化）
- 图像预处理 20 个算子全部实现 OpenCV 算法（灰度/镜像/旋转/深度转彩色/尺寸/滤波/形态学/增强/二值化）
- 预处理配置窗口：动态参数面板 + "执行"实时预览结果 + "确定"保存参数到节点
- 采集图像配置窗口：选择文件/文件夹，修复首次加载 bug，"确定"保存当前图像
- 右侧大界面：QOpenGLWidget 替换为 ImageViewWidget，流程运行结果实时显示
- 节点执行完成即把输出图像推到右侧大界面（nodeFinished 信号驱动）
- 端到端流程验证：源 -> 图像预处理 -> 汇，数据线性串联成功，灰度输出单通道

2026/8/30 新增项目 Skills 文档

- 新增 PROJECT_SKILLS.md：汇总项目架构、环境路径、构建命令、验证方法、新增算子指南、
  操作注意事项与待办事项，供 AI 助手下次接手时快速恢复上下文
